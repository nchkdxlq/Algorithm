...
/***********************************************
 ...
 这会导致递归的层数很深，最坏的情况层数为n，此时就退化成 O(n^2) 级别时间复杂度的算法。
...
int __partition2Ways(T arr[], int l, int r) {
    ...
    /* 循环结束后，i 在 >= v 的第一个位置，后半部分的第一个位置，此时 arr[i] 可能等于v， 也可能大于v；
     j在前半部分的最后一个位置, 此时 arr[j] 可能等于v，也可能小于v; 为了保证，arr[l...i]<=v, arr[j...r]>=v的特性，
     arr[l] 只能跟 arr[j] 交互位置，不能与arr[i]交换位置。j就是要找的那个下标。
     */
    swap(arr[l], arr[j]);
